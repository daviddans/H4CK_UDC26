...
export async function GET(
  ...
  try {
    const buffer = await readFile(entry.saved_path);
    const url = new URL(req.url);
    const disposition =
      url.searchParams.get("disposition") === "attachment" ? "attachment" : "inline";
    const safeName = entry.original_name || path.basename(entry.saved_path);

    return new Response(buffer, {
      status: 200,
      headers: {
        "Content-Type": getMimeType(entry.saved_path),
